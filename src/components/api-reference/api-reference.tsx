...
const repository = `${gitConfig.user}/${gitConfig.repo}`;
...
function githubSourceUrl(source: string) {
  if (source.startsWith("src/") && source.endsWith(".d.ts")) {
    const implementationPath = source.replace(/\.d\.ts$/, ".ts");
    return `https://github.com/${repository}/blob/${gitConfig.branch}/${implementationPath}`;
  }
...
function summaryFor(entry: ApiEntry) {
  return (
    entry.summary ||
    `${apiKindSingleLabel[entry.kind]} · ${apiPackage.name} ${apiPackage.version}`
  );
}
...
function KindBadge({ kind }: { kind: ApiKind }) {
  return (
    <span
      className={`inline-flex h-6 items-center rounded-md border px-2 text-[11px] font-semibold ${apiKindStyles[kind].badge}`}
    >
...
function EntryRow({ entry }: { entry: ApiEntry }) {
  return (
    <Link
      ...
      className="hoshimi-lift group grid gap-1 rounded-lg border border-transparent px-3 py-2 hover:border-fd-primary/40 hover:bg-fd-accent/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fd-ring"
    >
...
function ApiIndex() {
  return (
    <div className="hoshimi-animate-in not-prose space-y-8">
...
function ApiKindPage({ kind }: { kind: ApiKind }) {
  ...
  return (
    <div className="hoshimi-animate-in not-prose space-y-6">
      <section
        id="overview"
...
function DocTags({
  ...
  const returnTags = visibleTags.filter(
  ...
  const otherTags = visibleTags.filter(
    (tag) =>
      !["deprecated", "param", "returns", "return", "example", "link"].includes(
        tag.name,
      ),
  );
  ...
  return (
    ...
    >
      {!compact && <DocTagHeading level={2}>JSDoc</DocTagHeading>}
          ...
          <dl className="grid gap-1">
            {paramTags.map((tag) => {
              ...
              return (
                <div
                  key={tag.key}
                  className="grid gap-1 text-sm sm:grid-cols-[minmax(7rem,12rem)_1fr]"
                >
                  <dt>
                    {parts.name ? (
                  ...
                  <dd className="leading-5 text-fd-muted-foreground">
                    {parts.description}
                  </dd>
                </div>
              );
...
function CodeToken({
  ...
  return (
    <span
      className={children === highlightName ? "font-semibold" : undefined}
      style={style}
    >
      {children}
    </span>
...
const propertyMemberKinds = new Set([
  "Property",
  "Getter",
  "Setter",
  "IndexSignature",
...
function memberId(member: IndexedMember) {
  return (
    member.anchorId ??
    `member-${member.kind}-${member.name}-${member.index}`.replace(
      /[^A-Za-z0-9_-]/g,
      "-",
    )
  );
  ...
  return (
    <section className="rounded-lg border border-fd-border bg-fd-background">
      ...
      {members.length > 0 ? (
        ...
        <p className="px-4 py-5 text-sm text-fd-muted-foreground">
    ...
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-fd-muted-foreground">
          This export does not declare public properties or methods in the
    ...
    <div className="not-prose divide-y divide-fd-border rounded-lg border border-fd-border">
      {members.map((member) => (
        <article
          id={memberId(member)}
          key={memberId(member)}
          className="scroll-mt-24 space-y-2 p-4"
        >
          <div className="flex flex-wrap items-center gap-2">
            ...
            {member.kind !== memberDefaultKind[tone] && (
              <span
                className={`rounded-md border px-1.5 py-0.5 text-[11px] font-medium ${memberToneStyles[tone]}`}
              >
                {member.kind}
              </span>
            )}
...
function MixinTooltip() {
  return (
    <span className="group relative inline-flex shrink-0">
      ...
      >
        Mixins are helper types merged into this class by TypeScript
        declarations; their members are included even when the class only shows
        an extends clause.
      </span>
    </span>
  );
...
function entryTocItems(entry: ApiEntryDetail) {
  ...
  return [
    ...
    ...(entry.privateTypes.length > 0
      ? [{ title: "Private types", url: "#private-types" }]
      : []),
    ...
    ...(groups.methods.length > 0
      ...
      : []),
    ...(groups.other.length > 0
      ? [{ title: "Other Members", url: "#other-members" }]
      : []),
  ];
}

// API slugs handled by the shared `/docs/[[...slug]]` route, prefixed with `api`.
export function apiReferenceStaticParams() {
  return [
    ["api"],
    ...apiKindOrder.map((kind) => ["api", apiKindSlug[kind]]),
    ...featuredEntries.map((entry) => ["api", entry.slug]),
  ];
}

export function apiReferenceMetadata(apiSlug: string[]): Metadata {
...
export async function ApiReference({ apiSlug }: { apiSlug: string[] }) {
  const target = getApiTarget(apiSlug);
  ...
  return (
    <DocsPage toc={toc(tocItems)} full>
      <DocsTitle
        id="_top"
